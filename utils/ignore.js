...
register('command', (...args) => {
    ChatLib.chat(`/bi ${args.toString()}`)
}).setName('bi');

registerWhen('chat', timeThis('', (event) => {
    const message = ChatLib.getChatMessage(event, true);
    let messageArray = message.split(' ');
}), () => getInSkyblock() && World.isLoaded()).setCriteria('https://').setContains();

// register('command', (...args) => {
//     const argsRegex = /(.+) (.+)? (.+)?/;
//     const argsMatch = args.match(argsRegex);
//     if (argsMatch) {
//         let action = argsMatch[1];
//         let name = argsMatch[2];
//         let reason = argsMatch[3];
//         if (action === 'add') {
//             baoIgnoreList.list.push([name, reason]);
//             ChatLib.chat(`${baoUtils.modPrefix} &aAdded ${name} to Ignore List for reason: ${reason}`)
